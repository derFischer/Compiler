    ...
    if ((i->u.MOVE.dst == NULL) && (i->u.MOVE.src == NULL))
    {
      ...
      if (src != NULL)
      {
        ...
        if (dst != NULL)
        {
          ...
          if ((src[1] == dst[1]) && (src[2] == dst[2]) && (src[3] == dst[3]))
            break;
        }
      }
    }
...
AS_instrList AS_rewrite(AS_instrList il, int framesize)
{
  ...
  while (il)
  {
    ...
    case I_MOVE:
    {
      if (instr->u.MOVE.src->head == F_RBP())
      {
        char inst[INSTRLENTH];
        sprintf(inst, "leaq %d(%rsp), `d0", framesize);
        il->head = AS_Oper(String(inst), instr->u.MOVE.dst, NULL, NULL);
      }
      break;
    }
    case I_OPER:
    {
      if(instr->u.OPER.src != NULL && instr->u.OPER.src->head == F_RBP())
      {
        int num;
        char ifMovq[INSTRLENTH];
        sscanf(instr->u.OPER.assem, "%s %d", ifMovq, &num);
        if(strcmp(ifMovq, "movq") == 0)
        {
          char inst[INSTRLENTH];
          sprintf(inst, "movq %d(%rsp), `d0", framesize + num);
          il->head = AS_Oper(String(inst), instr->u.OPER.dst, NULL, NULL);
        }
      }
      else if(instr->u.OPER.src != NULL && instr->u.OPER.src->tail != NULL && instr->u.OPER.src->tail->head == F_RBP())
      {
        int num;
        char src1[INSTRLENTH];
        char ifMovq[INSTRLENTH];
        sscanf(instr->u.OPER.assem, "%s %s %d", ifMovq, src1, &num);
        if(strcmp(ifMovq, "movq") == 0)
        {
          char inst[INSTRLENTH];
          sprintf(inst, "movq `s0, %d(%rsp)", framesize + num);
          il->head = AS_Oper(String(inst), NULL, instr->u.OPER.src, NULL);
        }
      }
      break;
    }
